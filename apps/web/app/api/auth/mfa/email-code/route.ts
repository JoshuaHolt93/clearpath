  ...
  try {
    ...
    if (!response.ok || !data) {
      return NextResponse.json(
        { message: apiErrorMessage(error, "A new email verification code could not be sent.") },
        { status: response.status },
      );
    }
    const result = mfaEmailCodeResultSchema.safeParse({ sent: data.sent, reason: data.reason });
    if (!result.success) {
