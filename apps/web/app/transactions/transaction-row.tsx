  ...
  return <article id={`transaction-${transaction.id}`} className={styles.transactionRow}>
    ...
    {expanded ? <div id={`transaction-details-${transaction.id}`} className={styles.transactionDetails}>
      <div className={styles.actionStrip}>
        <Link className={styles.textButton} href={`/category-rules?match_text=${encodeURIComponent(transaction.displayMerchant)}${transaction.category ? `&category_id=${transaction.category.id}` : ""}`}><Tags size={15} />Create Rule</Link>
